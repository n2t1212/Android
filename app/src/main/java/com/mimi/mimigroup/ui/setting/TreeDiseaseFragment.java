...
import com.mimi.mimigroup.ui.adapter.TreeDiseaseAdapter;
...
public class TreeDiseaseFragment extends BaseFragment{
    @BindView(R.id.rvTreeDisease)
    RecyclerView rvTreeDisease;
    TreeDiseaseAdapter adapter;
    private DBGimsHelper mDB;

    @Override
    protected int getLayoutResourceId() {
        return R.layout.fragment_tree_disease;
    }
    @Override
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        super.onViewCreated(view, savedInstanceState);
        adapter = new TreeDiseaseAdapter();
        mDB=DBGimsHelper.getInstance(getContext());

        rvTreeDisease.setLayoutManager(new LinearLayoutManager(getContext()));
        rvTreeDisease.setAdapter(adapter);
        onLoadDataSource();
    ...
    private void onLoadDataSource(){
        try{
            List<DM_Tree_Disease> lstTreeDisease=mDB.getAllTreeDisease();
            adapter.setTreeDiseaseList(lstTreeDisease);

            Toast.makeText(getContext(),"Có "+ Integer.toString(lstTreeDisease.size())+" được nạp..",Toast.LENGTH_SHORT).show();
        }catch (Exception ex){}
    }
    ...
    public void onDownloadClicked(final boolean isAll){
        ...
        final String mUrlGet=AppSetting.getInstance().URL_SyncDM(AppUtils.getImeil(getContext()),AppUtils.getImeilsim(getContext()),"DM_TREE_DISEASE",isAll);
        new SyncGet(new APINetCallBack() {
            @Override
            public void onHttpStart() {
                ((BaseActivity) getActivity()).showProgressDialog("Đang tải dữ liệu dịch hại.");
                Log.d("URL_SYNC_TREE_DISEASE",mUrlGet);
            }
            ...
            public void onHttpSuccess(String ResPonseRs) {
                if( ResPonseRs==null || ResPonseRs.toString().trim()==null || ResPonseRs.isEmpty()) {
                    ...
                    try {
                        ...
                        Gson gson = new Gson();
                        Type type = new TypeToken<Collection<DM_Tree_Disease>>() {}.getType();
                        Collection<DM_Tree_Disease> enums = gson.fromJson(ResPonseRs, type);
                        DM_Tree_Disease[] ArrTreeDis = enums.toArray(new DM_Tree_Disease[enums.size()]);


                        if ( ArrTreeDis!=null && ArrTreeDis.length>0) {
                            new AsyncUpdate(new SyncCallBack() {
                                @Override
                                public void onSyncStart() {
                                    if(isAll) {
                                        mDB.delTreeDisease("", true);
                                    }
        ...
        }, mUrlGet, "DWN_TREE_DISEASE").execute();
    ...
    private class AsyncUpdate extends AsyncTask<DM_Tree_Disease[],String,String> {
        ...
        @Override
        protected String doInBackground(DM_Tree_Disease[]... params) {
            try {
                int iSize = params[0].length;
                int iSqno = 1;
                for (DM_Tree_Disease oItem : params[0]) {
                    if (!oItem.getDiseaseName().isEmpty()) {
                        mDB.addTreeDisease(oItem);
                        publishProgress("[" + Integer.toString(iSqno) + "/" + Integer.toString(iSize) + "] " + oItem.getDiseaseName());
                    }
                    iSqno += 1;
                }
            } catch (Exception ex) {
