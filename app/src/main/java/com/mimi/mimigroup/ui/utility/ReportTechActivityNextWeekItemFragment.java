...
import com.mimi.mimigroup.ui.adapter.ReportTechActivityNextWeekAdapter;
...
public class ReportTechActivityNextWeekItemFragment  extends BaseFragment {

    @BindView(R.id.rvReportTechActivityNextWeekList)
    RecyclerView rvReportTechActivityNextWeekList;
    ...
    @BindView(R.id.Layout_ReportTechActivityNextWeekItem)
    LinearLayout Layout_ReportTechActivityNextWeekItem;

    ReportTechActivityNextWeekAdapter adapter;
    ...
    protected int getLayoutResourceId() {
        return R.layout.fragment_report_tech_activity_next_week_form;
    }

    @Override
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        super.onViewCreated(view, savedInstanceState);
        adapter = new ReportTechActivityNextWeekAdapter(new ReportTechActivityNextWeekAdapter.ListItemClickListener() {
            @Override
        ...
        rvReportTechActivityNextWeekList.setLayoutManager(new LinearLayoutManager(getContext()));
        rvReportTechActivityNextWeekList.setAdapter(adapter);
        ...
        new android.os.Handler().postDelayed(
                new Runnable() {
                    public void run() {
                        ...
                        lstReportTechActivity = oActivity.getoReportTechActivityNextWeek();
    ...
    // CALL FROM ACTIVITY
    public List<SM_ReportTechActivity> getListReportTechActivityNextWeek() {
    ...
    private void setReportTechActivityRow(SM_ReportTechActivity osmDT) {
        try {
            if (osmDT != null) {
                ...
                Toast.makeText(getContext(), "Không tồn tại báo cáo hoạt động tuần tới..", Toast.LENGTH_SHORT).show();
            }
    ...
    public boolean onAddReportTechActivity(boolean isAddnew) {
        ((ReportTechFormActivity) getActivity()).setVisibleDetailDelete(false);
        if (Layout_ReportTechActivityNextWeekItem.getVisibility() == View.GONE) {
            Layout_ReportTechActivityNextWeekItem.setVisibility(View.VISIBLE);
            //Clear Input
            try {
                if (isAddnew) {
                    tvTitle.setText("");
    ...
    private boolean onSaveAddReportTechActivity() {
        ...
        for (int i = 0; i < lstReportTechActivity.size(); i++) {
            if (lstReportTechActivity.get(i).getActivitieId().equalsIgnoreCase(oDetail.getActivitieId())) {
                ...
                Toast.makeText(getContext(), "Báo cáo hoạt động tuần tới đã được thêm..", Toast.LENGTH_SHORT).show();
                break;
            }
        }
        ...
        if (!isExist) {
            ...
            String mId = "BCHDTTT" + mParSymbol + Od.format(new Date());
        ...
        adapter.setsmoReportTechActivity(lstReportTechActivity);
        Toast.makeText(getContext(), String.valueOf(lstReportTechActivity.size()) + ": Báo cáo hoạt động tuần tới được chọn..", Toast.LENGTH_SHORT).show();
    ...
    public void onDeletedReportTechActivity() {
        if (adapter.SelectedList == null || adapter.SelectedList.size() <= 0) {
            Toast oToat = Toast.makeText(getContext(), "Bạn chưa chọn báo cáo hoạt động tuần tới để xóa...", Toast.LENGTH_LONG);
